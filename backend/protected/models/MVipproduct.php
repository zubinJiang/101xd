...
class MVipproduct extends CModel {  
        ...
        if(!empty($data)){
            ...
            foreach($data as $k=>$v){
                $site_name = $this->db->table('`vipproduct_pusher` pu, `company` com')
                    ->select('distinct pu.pusher_id, com.site_name')
                    ->where("pu.user_id=com.user_id")
                    ->where("pu.pusher_id='{$user_id}'")
     ...
     * 获取vip用户推送的产品
     */
    function getVipproductList($user_id, $page, $num, $type=null){
     ...
     */
    function processVipproductData($state, $id, $mag=null){
    ...
    */
    ...
    /*根据vipproduct的id获取已经推送到的渠道商
     */
    function getPushedList($pid){
        $res = $this->db->table('`vipproduct_pusher`')
            ->select('*')
            ->where("vipproduct_id='{$pid}'")
            ->getList();
        ...
        foreach($res as $p){
            ...
            $product = $this->db->table('`vipproduct`')->select('*')->where("id='{$p['vipproduct_id']}'")->getFirst();
            
            if ($product['state']==0){
                echo $product;
                $user = $this->db->table('`user`')->select('*')->where("UserID='{$p['user_id']}'")->getFirst();
                array_push($pushedlist,$user);
            }
        }
        return $pushedlist;
    }

    /*根据vipproduct的id获取建议推送到的渠道商
     */
    function getSuggestPush($pid){
        $prod = $this->db->table('`vipproduct`')->select('*')->where("id={$pid}")->getFirst();
        /*此部分为匹配条件，开始时数据库的数据很少，注视掉
        $comps = $this->db->table('`company`')->select('*')->where("category_id={$prod['category_id']}")->getList();
        $pushlist = array();
        foreach($comps as $c){
            $da = $this->db->table('`user`')->select('*')->where("UserID='{$c['user_id']}'")->getFirst();
            array_push($pushlist,$da);
        }
         */
        $pushlist = $this->db->table('`user`')->select('*')->where("UserType=1")->getNum(30,0);
        return $pushlist;
    } 
