<?php
class VipsupplyPresenter extends InitPresenter
{
					...
					if ($_FILES["{$images}"]['size'] <= $fileSize) {
                        ...
                        $dirName = $dir.$name;
							if(move_uploaded_file($_FILES["{$images}"]['tmp_name'],$dirName)){
								...
								echo "上传图片上传文件失败！<br>";exit;
							}
					}else{
						echo "上传图片大小错误<br>";exit;
						
					}
				}else{
					echo "长传图片类型错误<br>";exit;	
    ...
    public function addData($user_id){
        ...
        if(!empty($vipproduct_rzt)){
            ...
            $this->redirect("/vipsupplySuccess?id={$company_id['company_id']}");
        } else {
