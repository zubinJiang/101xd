<?php
class VipPresenter extends InitPresenter
{
    ...
    public function index($id, $user_id){
        ...
        if($user_id!=$product['user_id']) {
            $role_id = $this->user->getUserRoleId($user_id);
            if($role_id<1 || $role_id>3) { exit('not auth!');}
        }
    ...
    private function seo($product) {
        ...
        $shi = (($id_%100)/10)%2;
        $bai = (($id_%1000)/100)%2;
        $keywords_base = array('货源','批发','b2b','供货','渠道','代销','供应商','批发');
